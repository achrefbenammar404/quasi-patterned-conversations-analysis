...
class Cluster:
    ...
    def assign_to_clusters(cluster_centers: np.ndarray, embeddings: List[List[float]]) -> List[int]:
        """
        ...
        Args:
            ...
            embeddings (List[List[float]]): list of embeddings to be clustered, shape (outer_dim, inner_dim)

        Returns:
            List[int]: a list of cluster ids assigned to each embedding vector, length same as embeddings.
        """
        ...
        distances = np.linalg.norm(embeddings_array[:, np.newaxis] - cluster_centers, axis=2)
        
        # Find the index of the minimum distance for each embedding
        assigned_clusters = np.argmin(distances, axis=1)
        
        return assigned_clusters.tolist()
    def elbow_method(embeddings: List[List[np.array]], min_clusters : int = 5 , max_clusters: int = 10, 
